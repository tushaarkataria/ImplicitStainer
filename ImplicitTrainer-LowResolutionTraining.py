...
from test_mine import eval_psnr
from skimage import io
from test_mine import eval_psnr_mine_64,eval_psnr_mine_high_resolution
...
def train(args,train_loader, model, optimizer):
    ...
    for batch in train_loader:
        ...
        gt_image = batch['gt'].view(batch_size,64,64,3).permute(0, 3, 1, 2).contiguous()
        predicted_image = pred.view(batch_size,64,64,3).permute(0, 3, 1, 2).contiguous()
...
def main(args,config_, save_path):
    ...
    model = model.cuda()
    print("**** Running EVAL********")
    res = eval_psnr_mine_64(test_loader, model,
        save_path_results,
    ...
    print("**** Running Inference High Resolution********")

    spec = config['test_dataset']
    dataset = datasets.make(spec['dataset'])
    dataset = datasets.make(spec['wrapper'], args={'dataset': dataset})
    save_path_results = os.path.join(save_path,'ResultsHighResolution')
    if not os.path.exists(save_path_results):
        os.makedirs(save_path_results)
    ...
    model = model.cuda()
    print("**** Running EVAL********")
    res = eval_psnr_mine_high_resolution(test_loader, model,
        save_path_results,
        data_norm=config.get('data_norm'),
        eval_type=config.get('eval_type'),
        eval_bsize=None,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config' , default='configs/train-he2ihc/train_he_to_ihc_liif_64.yaml')
    ...
    parser.add_argument('--local_window_size', default=3,type=int)
    ...
    elif(args.backbone=='swin-conv-parallel-add-l-64'):
        config['model']['args']['encoder_spec']['name']='swin-conv-parallel-add-l-64'
    
    if(args.dataPercentage=='tenth'):
        config['train_dataset']['dataset']['name'] = 'paired-image-folders-downsample-64-10'
