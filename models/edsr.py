...
class EDSRWithUpsampling(nn.Module):
    def __init__(self, args, conv=default_conv):
        super(EDSRWithUpsampling, self).__init__()
        self.args = args
...
class EDSRDecoder(nn.Module):
    def __init__(self, args, conv=default_conv):
        super(EDSRDecoder, self).__init__()
        self.args = args
        n_resblocks = args.n_resblocks
        n_feats = args.n_feats
        kernel_size = 3
...
class EDSRDecoderHalf(nn.Module):
    def __init__(self, args, conv=default_conv):
        super(EDSRDecoderHalf, self).__init__()
        self.args = args
        n_resblocks = args.n_resblocks
        n_feats = args.n_feats
        kernel_size = 3
        ...
        # define head module
        m_head = [conv(n_feats, n_feats//2, kernel_size)]
...
class EDSRDecoderQuater(nn.Module):
    def __init__(self, args, conv=default_conv):
        super(EDSRDecoderQuater, self).__init__()
        self.args = args
        n_resblocks = args.n_resblocks
        n_feats = args.n_feats
        kernel_size = 3
        ...
        # define head module
        m_head = [conv(n_feats, n_feats//4, kernel_size)]
        ...
        self.tail = conv(n_feats//4, args.n_colors, kernel_size)
...
@register('edsr-smaller')
def make_edsr_baseline_smaller(n_resblocks=16, n_feats=16, res_scale=1,
...
@register('edsr-small')
def make_edsr_baseline_small(n_resblocks=32, n_feats=32, res_scale=1,
                       scale=2, no_upsampling=False, rgb_range=1):
    args = Namespace()
    args.n_resblocks = n_resblocks
    args.n_feats = n_feats
